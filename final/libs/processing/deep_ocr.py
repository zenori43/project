...
def correct_ocr_duplicate_slash(text: str) -> str:
    """แก้ OCR: /25/25 -> /25, /26/26 -> /26 และ DD/00/YY (เดือน) -> DD/10/YY"""
    if not text or not isinstance(text, str):
        return text
    # แทนที่ /XX/XX เป็น /XX (เลขซ้ำหลัง slash)
    s = re.sub(r'/(\d+)/\1\b', r'/\1', text)
    # ในบริบทวันที่ ถ้าเดือนเป็น 00 แก้เป็น 10 (เช่น BBF19/00/25 -> BBF19/10/25)
    s = re.sub(r'(\d{1,2})/00/(\d{1,2})\b', r'\1/10/\2', s)
    return s


# รูปแบบ cap 3 บรรทัด: บรรทัด1 MFGxx/xx/xx, บรรทัด2 BBFxx/xx/xx, บรรทัด3 xx:xxSxx
_CAP_LINE1_PATTERN = re.compile(r'(?:MFG|mfg)\s*([0-9/]+)', re.IGNORECASE)
_CAP_LINE2_PATTERN = re.compile(r'(?:BBF|bbf)\s*([0-9/]+)', re.IGNORECASE)
_CAP_LINE3_PATTERN = re.compile(r'(\d{1,2})\s*:\s*(\d{1,2})\s*S\s*(\d{1,2})', re.IGNORECASE)


def _format_date_dd_mm_yy(nums: str, force_yy: Optional[str] = None) -> str:
    """
    แปลงสายตัวเลข+slash เป็น dd/mm/yy (8 ตัว).
    force_yy ถ้าส่งมา (เช่น "26") ใช้เป็นปีแทนที่อ่านได้ — อิงจากบรรทัด MFG/BBF อีกบรรทัด
    """
    def apply_yy(dd: str, mm: str, yy: str) -> str:
        if force_yy and len(force_yy) >= 2:
            yy = force_yy[-2:].zfill(2)
        elif yy == '00':
            yy = '10'
        elif yy in ('05', '06', '07') and not force_yy:
            yy = {'05': '25', '06': '26', '07': '27'}.get(yy, yy)
        return f'{dd}/{mm}/{yy}'

    nums = (nums or '').strip()
    digits_only = re.sub(r'[^0-9]', '', nums)
    # มี slash อยู่แล้ว แบบ dd/mm/yy
    if re.match(r'^\d{1,2}/\d{1,2}/\d{1,2}$', nums):
        parts = nums.split('/')
        dd, mm, yy = parts[0].zfill(2), parts[1].zfill(2), parts[2].zfill(2)
        return apply_yy(dd, mm, yy)
    # มี slash หนึ่งตัว เช่น 23009/05 หรือ 23007/06
    if '/' in nums and nums.count('/') == 1:
        a, b = nums.split('/', 1)
        da = re.sub(r'[^0-9]', '', a)
        db = re.sub(r'[^0-9]', '', b)
        if len(da) >= 4 and len(db) >= 2:  # 23007, 06
            dd, mm = da[:2], da[3:5] if len(da) >= 5 else da[2:4]
            yy = db[:2]
            return apply_yy(dd, mm, yy)
        if len(da) >= 2 and len(db) >= 4:
            dd = da[:2]
            mm, yy = db[:2], db[2:4]
            return apply_yy(dd, mm, yy)
    # ไม่มี slash - ใช้เฉพาะตัวเลขเรียง
    D = digits_only
    if len(D) >= 6:
        if len(D) == 6:
            dd, mm, yy = D[0:2], D[2:4], D[4:6]
        elif len(D) == 7:
            dd, mm, yy = D[0:2], D[3:5], D[5:7]
        else:
            dd, mm, yy = D[0:2], D[2:4], D[6:8]
        if mm == '00':
            mm = '10'
        return apply_yy(dd, mm, yy)
    if len(nums) >= 6:
        return nums[:8] if len(nums) >= 8 else nums
    return nums or '01/01/01'


def _normalize_cap_line1(text: str, ref_yy: Optional[str] = None) -> str:
    """ดึงเฉพาะรูปแบบ MFGxx/xx/xx และจัดเป็น dd/mm/yy; ref_yy = ปีจากบรรทัด BBF (อิงกัน)"""
    if not text or not isinstance(text, str):
        return ''
    m = _CAP_LINE1_PATTERN.search(text)
    if m:
        nums = m.group(1).strip()
    else:
        if 'MFG' not in text.upper():
            return 'MFG01/01/01'
        rest = text.upper().split('MFG', 1)[-1]
        nums = rest.strip()
    nums = re.sub(r'[^0-9/]', '', nums)
    if not nums:
        return 'MFG01/01/01'
    formatted = _format_date_dd_mm_yy(nums, force_yy=ref_yy)
    if re.match(r'^\d{2}/\d{2}/\d{2}$', formatted):
        return 'MFG' + formatted
    if len(nums) >= 8:
        nums = nums[:8]
    nums = re.sub(r'^(\d{1,2})/00/(\d{1,2})', r'\1/10/\2', nums)
    return 'MFG' + nums if nums else 'MFG01/01/01'


def _normalize_cap_line2(text: str, ref_yy: Optional[str] = None) -> str:
    """ดึงเฉพาะรูปแบบ BBFxx/xx/xx และจัดเป็น dd/mm/yy; ref_yy = ปีจากบรรทัด MFG (อิงกัน)"""
    if not text or not isinstance(text, str):
        return ''
    m = _CAP_LINE2_PATTERN.search(text)
    if m:
        nums = m.group(1).strip()
    else:
        if 'BBF' not in text.upper():
            return 'BBF01/01/01'
        rest = text.upper().split('BBF', 1)[-1]
        nums = rest.strip()
    nums = re.sub(r'[^0-9/]', '', nums)
    if not nums:
        return 'BBF01/01/01'
    formatted = _format_date_dd_mm_yy(nums, force_yy=ref_yy)
    if re.match(r'^\d{2}/\d{2}/\d{2}$', formatted):
        return 'BBF' + formatted
    if len(nums) >= 8:
        nums = nums[:8]
    nums = re.sub(r'^(\d{1,2})/00/(\d{1,2})', r'\1/10/\2', nums)
    return 'BBF' + nums if nums else 'BBF01/01/01'


def _normalize_cap_line3(text: str) -> str:
    """ดึงเฉพาะรูปแบบ xx:xxSxx ถ้า S หาย (เช่น 20:320022) ให้แทรก S แล้วตัดเป็น 20:32S02"""
    if not text or not isinstance(text, str):
        return '00:00S00'
    # แก้กรณี S หาย: xx:xx ตามด้วยตัวเลขต่อกัน → xx:xxS + 2 หลักแรก (ที่เหลือตัดทิ้ง)
    no_s = re.match(r'^(\d{1,2})\s*:\s*(\d{1,2})\s*(\d{2})\d*$', text.strip())
    if no_s:
        a, b, c = no_s.group(1).zfill(2), no_s.group(2).zfill(2), no_s.group(3)
        return f'{a}:{b}S{c}'
    m = _CAP_LINE3_PATTERN.search(text)
    if m:
        a, b, c = m.group(1).zfill(2), m.group(2).zfill(2), m.group(3).zfill(2)
        return f'{a}:{b}S{c}'
    return '00:00S00'


def _extract_yy_from_mfg_bbf(s: str) -> Optional[str]:
    """ดึงปี 2 หลักจากข้อความ MFGxx/xx/xx หรือ BBFxx/xx/xx (ตัวท้ายของวันที่)"""
    if not s or len(s) < 2:
        return None
...
def normalize_cap_format(line_results: List[Dict]) -> List[Dict]:
    """
    บังคับผล cap เป็น 3 บรรทัด: MFGxx/xx/xx, BBFxx/xx/xx, xx:xxSxx
    MFG กับ BBF อิงปีกัน — ถ้าบรรทัดหนึ่งเป็น 26 อีกบรรทัดอ่านได้ 06 จะแก้เป็น 26
    """
    if not line_results:
        return line_results
    texts = [r.get('recognized_text', '') or '' for r in line_results]
    is_cap = (len(texts) >= 2 and 'MFG' in texts[0].upper() and 'BBF' in texts[1].upper())
    if not is_cap:
        return line_results
    out = []
    for i in range(3):
        if i < len(line_results):
            r = dict(line_results[i])
            if i == 0:
                r['recognized_text'] = _normalize_cap_line1(texts[0])
            elif i == 1:
                r['recognized_text'] = _normalize_cap_line2(texts[1])
            else:
                r['recognized_text'] = _normalize_cap_line3(texts[2])
            r['line_index'] = i
            out.append(r)
        else:
            out.append({
                ...
                'bbox': None,
                'num_characters': 0,
                'recognized_text': _normalize_cap_line3('') if i == 2 else (_normalize_cap_line1('') if i == 0 else _normalize_cap_line2('')),
                'confidence_score': 0.0,
                'error': None
            })
    # อิงปีระหว่างบรรทัด 1 กับ 2: ถ้าบรรทัดหนึ่ง 25/26/27 อีกบรรทัด 05/06/07 แก้ปีของบรรทัดที่ผิด
    yy1 = _extract_yy_from_mfg_bbf(out[0]['recognized_text'])
    yy2 = _extract_yy_from_mfg_bbf(out[1]['recognized_text'])
    small, big = ('05', '06', '07'), ('25', '26', '27')
    if yy1 in small and yy2 in big and yy1[1] == yy2[1]:
...
class DeepOCRModel:
    ...
    def __init__(self, model_path: str = None, cuda: bool = True):
        ...
        self.ocr_history = []
        self.ocr_history_max = 50  # เก็บสูงสุด 50 รายการ
    ...
    def recognize_text_from_image_array(self, image: np.ndarray, image_path: str = None) -> Dict:
        ...
        try:
            ...
            try:
                ...
                # Store in global variables (จำกัดจำนวน — ไม่เก็บ original_image ใน history ถ้าเกิน limit)
                self.last_ocr_result = result
                self.ocr_history.append(result)
                while len(self.ocr_history) > self.ocr_history_max:
                ...
                with torch.no_grad():
                    for image_tensors, image_path_list in demo_loader:
                        ...
                        for img_name, pred, pred_max_prob in zip(image_path_list, preds_str, preds_max_prob):
                            ...
                            pred = correct_ocr_duplicate_slash(pred)
                            results.append({
                                'image_path': img_name,
