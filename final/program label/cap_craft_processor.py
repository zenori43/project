...
# Make sure we can import from project root (final/).
...
# (core/cuda_setup.py prints unicode symbols like ⚠️ on import).
try:
...
from libs.detection.capmodel import CapDetector, initialize_detector as init_cap_detector
from libs.processing.rotationCRAFT import CRAFTTextDetector, initialize_detector as init_craft_detector


def _resolve_cap_craft_model_paths():
    """
    Paths มาตรฐานของโปรเจกต์ (เทียบกับ config/settings.py)
    - cap.pt: final/models/detection/cap.pt
    - CRAFT: project_final/CRAFT-pytorch/ หรือ final/external/CRAFT-pytorch/
    สามารถ override ด้วย env: CAP_MODEL_PATH, CRAFT_MODEL_PATH, CRAFT_REFINER_PATH
    """
    final_dir = str(FINAL_DIR)
    cap_default = os.path.join(final_dir, "models", "detection", "cap.pt")
    cap_model_path = os.environ.get("CAP_MODEL_PATH", cap_default)

    parent = Path(final_dir).resolve().parent
    craft_root = parent / "CRAFT-pytorch"
    craft_external = Path(final_dir) / "external" / "CRAFT-pytorch"
    if (craft_root / "craft_mlt_25k.pth").is_file():
        craft_dir = craft_root
    else:
        craft_dir = craft_external

    craft_model_path = os.environ.get(
        "CRAFT_MODEL_PATH", str(craft_dir / "craft_mlt_25k.pth")
    )
    craft_refiner_path = os.environ.get(
        "CRAFT_REFINER_PATH", str(craft_dir / "craft_refiner_CTW1500.pth")
    )
    return cap_model_path, craft_model_path, craft_refiner_path
...
class CapCraftProcessor:
    ...
    def initialize_detectors(self):
        ...
        try:
            ...
            cap_model_path, craft_model_path, craft_refiner_path = _resolve_cap_craft_model_paths()
            ...
            if os.path.exists(cap_model_path):
                ...
                self.log_info(f"   คาดหวังที่: {cap_model_path}")
                self.log_info("   วาง cap.pt ที่โฟลเดอร์ models/detection/ หรือตั้ง CAP_MODEL_PATH")

            # Initialize CRAFT detector (refiner ไม่บังคับ — ถ้าไม่มีไฟล์จะส่ง None)
            try:
                import torch
            ...
            refiner = craft_refiner_path if os.path.isfile(craft_refiner_path) else None
            if refiner is None and craft_refiner_path:
                self.log_info(f"⚠️ ไม่พบ refiner (ใช้ CRAFT แบบไม่มี refiner): {craft_refiner_path}")

            if os.path.exists(craft_model_path):
                self.craft_detector = init_craft_detector(
                    craft_model_path, refiner, cuda=use_cuda
                )
                self.log_info("✅ โมเดล CRAFT โหลดสำเร็จ")
            else:
                self.log_info("❌ ไม่พบไฟล์โมเดล CRAFT")
                self.log_info(f"   คาดหวังที่: {craft_model_path}")
                self.log_info(
                    "   วาง craft_mlt_25k.pth ใน external/CRAFT-pytorch/ หรือโฟลเดอร์ CRAFT-pytorch ข้างโปรเจกต์"
                )
