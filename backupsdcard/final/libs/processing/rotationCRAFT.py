...
def _import_craft_modules():
    ...
    try:
        ...
        try:
            # Try importing from basenet directory first (most common case)
            from basenet.vgg16_bn import vgg16_bn, init_weights
            print("✅ vgg16_bn imported from basenet directory")
        except ImportError as e:
            print(f"⚠️ basenet.vgg16_bn import failed: {e}")
            print("🔧 Trying direct import...")
            try:
                from vgg16_bn import vgg16_bn, init_weights
                print("✅ vgg16_bn imported directly")
            except ImportError as e2:
                print(f"❌ Direct import also failed: {e2}")
                # Try importing from basenet as a module
                try:
                    import basenet
                    vgg16_bn = basenet.vgg16_bn
                    init_weights = basenet.init_weights
                    print("✅ vgg16_bn imported from basenet module")
...
class CRAFTTextDetector:
    ...
    def __init__(self, model_path: str = None, refiner_path: str = None, cuda: bool = True):
        ...
        if model_path:
            success = self.load_model(model_path, refiner_path)
            if not success:
                raise ValueError(f"Failed to load CRAFT model from {model_path}. Please check if the file exists and is a valid CRAFT model.")
    ...
    def detect_text_and_rotate(self, image_path: str = None, image_array: np.ndarray = None) -> Dict:
        ...
        if self.model is None:
            print(f"❌ ERROR: CRAFT model is None in detect_text_and_rotate()")
    ...
    def draw_detections(self, image: np.ndarray = None, result: Dict = None) -> np.ndarray:
        ...
        # Draw text detection boxes (อย่าใช้ result['final_polys'] ใน boolean — อาจเป็น numpy array)
        fp = result.get('final_polys')
        if fp is not None and hasattr(fp, '__len__') and len(fp) > 0:
            for i in range(len(fp)):
                poly = fp[i] if not isinstance(fp, np.ndarray) else np.asarray(fp[i])
