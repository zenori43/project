...
class CapDetector:
    ...
    def __init__(self, model_path: str = None, conf_threshold: float = 0.5):
        ...
        if model_path:
            ...
            if not success:
                raise ValueError(f"Failed to load model from {model_path}. Please check if the file exists and is a valid YOLOv8 model.")
    ...
    def detect_caps(self, image_path: str = None, image_array: np.ndarray = None) -> Dict:
        ...
        if self.model is None:
            print(f"❌ ERROR: Model is None in detect_caps()")
        ...
        try:
            ...
            # บน CPU ใช้ 640 เพื่อความเร็วสูงสุด; บน GPU ใช้ 1280
            original_height, original_width = image.shape[:2]
            max_dimension = max(original_height, original_width)
            max_edge = 640 if self.device.type == 'cpu' else 1280
            if max_dimension > max_edge:
                scale_factor = float(max_edge) / max_dimension
                ...
                print(f"📐 Resized image for detection: {original_width}x{original_height} -> {new_width}x{new_height} (scale: {scale_factor:.3f}, max_edge={max_edge})")
            else:
                resized_image = image
                scale_factor = 1.0
...
def initialize_detector(model_path: str, conf_threshold: float = 0.5) -> CapDetector:
    ...
    if _global_cap_detector.model is None:
        raise ValueError(f"Failed to load model from {model_path}. Please check if the file exists and is a valid YOLOv8 model.")
