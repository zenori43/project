...
# Load YOLO bottle model and confidence from config
try:
    from config.settings import (
        BOTTLE_MODEL_PATH, BOTTLE_YOLO_CONF, BOTTLE_YOLO_CONF_FALLBACK,
        YOLO_IMGSZ as _YOLO_IMGSZ, YOLO_MAX_EDGE as _YOLO_MAX_EDGE,
    )
    BOTTLE_MODEL = BOTTLE_MODEL_PATH
    YOLO_IMGSZ = _YOLO_IMGSZ
    YOLO_MAX_EDGE = _YOLO_MAX_EDGE
except ImportError:
...
# YOLO_MAX_EDGE, YOLO_IMGSZ มาจาก config (หรือค่าข้างบนถ้า ImportError)
...
def _max_confidence_for_label(detections: List[Dict], label: str) -> Optional[float]:
    """ค่า confidence สูงสุดของกล่อง YOLO ที่ class ตรงกับ label"""
    confs = [float(d["confidence"]) for d in detections if d.get("label") == label]
...
def detect_bottle_and_crop_type(image: np.ndarray, confidence_threshold: float = None) -> Dict:
    ...
    ถ้ารอบแรกได้ 0 detections จะลองรันอีกครั้งด้วย BOTTLE_YOLO_CONF_FALLBACK (conf ต่ำลง)
    """
    conf = confidence_threshold if confidence_threshold is not None else BOTTLE_YOLO_CONF
    h, w = image.shape[:2]
    scale_factor = 1.0
    img_for_yolo = image
    ...
    detections = detect_bottles(img_for_yolo, conf, imgsz=YOLO_IMGSZ)
    # ถ้าไม่เจออะไรเลย ลองรันอีกครั้งด้วย confidence ต่ำลง (fallback)
    if len(detections) == 0 and conf == BOTTLE_YOLO_CONF and BOTTLE_YOLO_CONF_FALLBACK < conf:
        print(f"🔄 BOTTLE YOLO: ไม่เจอที่ conf={conf} — ลองรันอีกครั้งที่ conf={BOTTLE_YOLO_CONF_FALLBACK}")
        detections = detect_bottles(img_for_yolo, BOTTLE_YOLO_CONF_FALLBACK, imgsz=YOLO_IMGSZ)
        if len(detections) > 0:
            print(f"✅ BOTTLE YOLO: เจอ {len(detections)} ตัวที่ conf={BOTTLE_YOLO_CONF_FALLBACK}")
    if scale_factor != 1.0:
        detections = [
    ...
    result_summary: Dict[str, Any] = {
        ...
        'angle1_confidence': _max_confidence_for_label(detections, 'angle1'),
        'type_confidence': _max_confidence_for_label(detections, 'type'),
    }
